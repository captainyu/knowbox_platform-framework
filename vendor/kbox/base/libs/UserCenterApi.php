...
class UserCenterApi{
    ...
    public static function findListByWhere($where,$onlyPlat,$where2=[]){
        ...
        $apiUrl = $onlyPlat ? static::API_USERLIST : static::API_USERLIST_All;
        $userList = static::curlCenter($apiUrl,$data);
        if(empty($userList)){
            return [];
        }
    ...
    public static function findPageListByWhere($page,$pagesize,$where=[],$where2=[]){
        ...
        $apiUrl = static::API_USERLIST_PAGE;
        $userList = static::curlCenter($apiUrl,$data);
        return $userList;
    }
    ...
    public static function findOneByWhere($where,$onlyPlat,$where2=[]){
        $userList = static::findListByWhere($where,$onlyPlat,$where2);
        return isset($userList[0]) ? $userList[0] : [];
    }

    public static function findOneByMobile($mobile,$onlyPlat){
        $where = ['mobile'=>$mobile];
        return static::findOneByWhere($where,$onlyPlat);
    }
